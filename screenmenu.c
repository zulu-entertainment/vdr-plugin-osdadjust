...
cScreenMenu::cScreenMenu()
{
...
#endif  
  width = Width;
  if (config.VideoFormat == 0 || width >= 1025)
     height = Height;
  else
  ...
  if (MAXOSDWIDTH > width) {
     width = MAXOSDWIDTH;
     height = MAXOSDHEIGHT;
     }

  if (config.AspectRatio == 0)
     aspect = Setup.VideoFormat;
#endif

  dsyslog("osdadjust: max OSD size is %dx%d", width, height);
...
//        osd->Flush();
  ...
  if (width < 1025) {
	  ...
	  for (int i = 0; i < 60; i++)
		  osd->DrawRectangle(w*i,  0, w*(i+1)-1,    h-1, 0xff000000+0x040404*(60-i));
	  for (int i = 0; i < 60; i++)
		  osd->DrawRectangle(w*i,  h, w*(i+1)-1,  2*h-1, 0xff000000+((15+i*4)<<16));
	  for (int i = 0; i < 60; i++)
		  osd->DrawRectangle(w*i, 2*h, w*(i+1)-1, 3*h-1, 0xff000000+((15+(60-i)*4)<<8));
	  for (int i = 0; i < 60; i++)
		  osd->DrawRectangle(w*i, 3*h, w*(i+1)-1, 4*h-1, 0xff000000+(15+i*4));
	  for (int i = 0; i < 60; i++)
		  osd->DrawRectangle(w*i, 4*h, w*(i+1)-1, 5*h-1, 0xff000000+0x040404*(60-i));    
  }
...
void cScreenMenu::DrawCycle(int tmpFgColor, int tmpBgColor)
{
  ...
  if (config.AspectRatio == 2 || (config.AspectRatio == 0 && aspect == 1))
     offset = ((768 - 720) / 2 + (1024 - 768) / 2) / 2;   // 16:9
  if (config.AspectRatio == 3 ||  width >= 1025)
     offset = 0;    // 1:1 || hd
...
void cScreenMenu::DrawGrid(int tmpFgColor, int tmpBgColor)
{
  ...
  if (config.AspectRatio == 2 || (config.AspectRatio == 0 && aspect == 1)) x = 32;    // 16:9
  if (config.AspectRatio == 3 ||  width >= 1025) x = 40;    // 1:1
  if (width >= 1025) offset = 0;
  else offset = 12;
  ...
  offset = 1;    // 4:3
  if (config.AspectRatio == 2 || (config.AspectRatio == 0 && aspect == 1)) offset = 8;    // 16:9
  if (config.AspectRatio == 3 ||  width >= 1025) offset = 0;    // 1:1
...
//  int currentRow = height / 2;      // horizont
  int myTab = 8 * font->Height();   // workaround
...
*/
  ...
  // Draw info
  snprintf(buffer, sizeof(buffer), "%s: %d", tr("Top"), y0);
  ...
  snprintf(buffer, sizeof(buffer), "%s: %d", tr("Left"), x0);
  ...
  snprintf(buffer, sizeof(buffer), "%s: %d", tr("Width"), x1 - x0);
  osd->DrawText(x0 + 50, y0 + 50 + 2*lineHeight, buffer, myFgColor, myTransparent, font);

  snprintf(buffer, sizeof(buffer), "%s: %d", tr("Height"), y1 - y0);
...
#if APIVERSNUM >= 10707
     Setup.OSDLeftP   = double(100 * Setup.OSDLeft / width) / 100;
     Setup.OSDTopP    = double(100 * Setup.OSDTop / height) / 100;
     Setup.OSDWidthP  = double(100 * Setup.OSDWidth / width) / 100;
     Setup.OSDHeightP = double(100 * Setup.OSDHeight / height) / 100;
#endif
